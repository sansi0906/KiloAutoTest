...
class JeecgBootClient(BaseClient):
    ...
    def worker_sign_save(self, user_uuid, worker_phone, station_info_id, station_info_name, province_area_code, city_area_code, district_area_code, source_type, service_items):
        """入驻超级个体

        Args:
            user_uuid: 工人ID
            worker_phone: 工人手机号
            station_info_id: 驿站ID
            station_info_name: 驿站名称
            province_area_code: 省区域编码
            city_area_code: 市区域编码
            district_area_code: 区县区域编码
            source_type: 订单来源
            service_items: 服务项目列表，每个元素包含 serviceItemId, itemName, billingMethod, amount

        Returns:
            Response 对象
        """
        return self.post("/api/worker-sign/save", json={
            "userUuid": user_uuid,
    ...
    def worker_save(self, name, phone, cert_num, cert_front_photo, cert_back_photo, address, nation=None, sex=None, birth=None, authority=None, valid_start_date=None, valid_end_date=None, verify_date=False, user_uuid=None):
        """用户注册提交信息（超级个体工人信息保存）
        ...
        Args:
            ...
            phone: 手机号
            cert_num: 身份证号
            cert_front_photo: 身份证正面图片路径
            cert_back_photo: 身份证反面图片路径
            address: 地址
            nation: 民族（可选）
            sex: 性别（可选）
            birth: 出生日期（可选）
            authority: 签发机关（可选）
            valid_start_date: 有效期开始时间（可选）
            valid_end_date: 有效期结束时间（可选）
            verify_date: 有效期是否永久（可选，默认False）
            user_uuid: 用户UUID（可选）
        ...
        if valid_start_date is not None:
            payload["validStartDate"] = valid_start_date
        if valid_end_date is not None:
            payload["validEndDate"] = valid_end_date
        return self.post("/api/worker-sign/worker-save", json=payload)

    def worker_info(self, user_uuid=None, phone=None):
        """查询用户信息
        ...
        if phone is not None:
            payload["phone"] = phone
        return self.post("/api/worker-sign/worker-info", json=payload)

    def worker_authorization(self, user_uuid=None, phone=None):
        """用户信息授权
        ...
        if phone is not None:
            payload["phone"] = phone
        return self.post("/api/worker-sign/worker-authorization", json=payload)

    def gongrenle_station_list(self, user_uuid=None, name=None, page=1, limit=10):
        """工人乐获取站点列表
        ...
        if limit is not None:
            payload["limit"] = limit
        return self.post("/api/worker-sign/gongrenle-station-list", json=payload)

    def gongrenle_station_detail(self, business_id):
        """工人乐获取站点详情
        ...
        Returns:
            Response 对象
        """
        return self.post(f"/api/worker-sign/gongrenle-station-detail?businessId={business_id}")

    def gongrenle_info(self, phone):
        """工人乐查询用户详情
        ...
        Returns:
            Response 对象
        """
        return self.post(f"/api/worker-sign/gongrenle-info?phone={phone}")